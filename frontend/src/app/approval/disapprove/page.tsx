...
export default function ApproveReview({ params }: { params: { rid: string } }) {
  ...
  const disapprove = async () => {
    const disapproveReview = await editApprove(
      session.user.token,
      params.rid,
      "disapproved"
    );
    ...
    <main>
      <div className="flex min-h-full w-auto flex-1 flex-col justify-center rounded-3xl px-6 py-12 md:px-15 md:mx-20 lg:mx-[200px]">
        ...
        <div className="flex justify-center items-center">
          ...
          <button
            onClick={refresh}
            className="block rounded-md bg-black hover:bg-indigo-900 px-3 py-2 text-white shadow-sm flex flex-row m-10"
          >
            Back to Menu
          </button>
          {/* </Link> */}
