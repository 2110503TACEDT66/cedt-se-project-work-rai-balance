      ...
      <div>
        <div className="flex min-h-full w-auto flex-1 flex-col justify-center rounded-3xl px-6 py-12 md:px-15 md:mx-20 lg:mx-[200px]">
                <div className="bg-white p-5 rounded-3xl drop-shadow-xl w-auto text-black">
                    <div className="text-2xl pt-5 pl-7 mb-5">{users.data.name}</div>
                    <hr className="bg-gray-200 border" />
                        <table className="table-auto border-separate border-spacing-2 px-5 ">
                            <tbody >
                                <tr><td>Email</td><td>{'    '}</td><td>{users.data.email}</td></tr>
                                <tr><td>Tel.</td><td>{'    '}</td><td>{users.data.telephone}</td></tr>
                                <tr><td>Tel.</td><td>{'    '}</td><td>{users.data.telephone}</td></tr>

                                {/* <tr><td>Member Since</td><td>{'    '}</td><td>{}</td></tr> */}
                                <tr><td>Point</td><td>{'    '}</td><td>{users.data.currentPoint}</td></tr>
                            </tbody>
                        </table>
                        
                              {users.data.reservations.map((reservation: UserBookingItem2) => (
                                <div className="bg-slate-100 p-5 rounded-xl w-[80%] text-black ml-4 mb-2">
                                    <tbody key={reservation._id} >
                                      <tr><td>BookingID</td><td>{'    '}</td><td>{reservation._id}</td></tr>
                                      <tr><td>Date</td><td>{'    '}</td><td>{reservation.apptDate}</td></tr>
                                      <tr><td>Start</td><td>{'    '}</td><td>{reservation.start}</td></tr>
                                      <tr><td>End</td><td>{'    '}</td><td>{reservation.end}</td></tr>
                                    </tbody>
                                      <div className="bg-slate-200 p-5 rounded-xl w-[80%] text-black ml-4 mt-2 mb-2">
                                          {reservation.reviews.map((review: ReviewItem4) => (
                                            
                                              <tbody key={review._id}>
                                              <tr><td>ReviewID</td><td>{'    '}</td><td>{review._id}</td></tr>
                                              <tr><td>Rating</td><td>{'    '}</td><td>{review.rating}</td></tr>
                                              <tr><td>Comment</td><td>{'    '}</td><td>{review.comment}</td></tr>
                                              <tr className="text-red-600"><td>Status</td><td>{'    '}</td><td>{review.approval}</td></tr>
                                            </tbody>
                                          ))}
                                      </div>
                                </div>
